...
class HeterogeneousAlgorithm extends BaseAlgorithm {
  constructor() {
    super({
      ...
      fileRequirements: {
        count: 3,
        types: ['graph', 'path', 'properties'],
        extensions: ['.txt'],
        descriptions: {
          graph: 'Graph structure and topology',
          path: 'Path information and connectivity',
  ...
  validateFiles(files) {
    const required = ['graphFile', 'pathFile', 'propertiesFile'];
    ...
    for (const [fileType, file] of Object.entries(files)) {
      if (required.includes(fileType) && file) {
        if (!file.name.toLowerCase().endsWith('.txt')) {
          return {
            valid: false,
            error: `${fileType} must be a .txt file`
          };
        }
      }
    }
  ...
  async process(files, parameters) {
    ...
    // Extract dataset name from graph file (remove extension)
    const datasetName = files.graphFile.name.replace('.txt', '');
  ...
  estimateProcessingTime(files, parameters) {
    ...
    let totalSize = 0;
    ['graphFile', 'pathFile', 'propertiesFile'].forEach(fileType => {
      if (files[fileType]) {
